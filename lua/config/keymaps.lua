...
local ts_js_ns = vim.api.nvim_create_namespace("ts_js_diagnostics")
  ...
  if focus_mode_enabled then
    ...
    vim.api.nvim_create_autocmd("CursorMoved", {
      ...
      callback = function()
        ...
        local ts_js_diags = vim.diagnostic.get(bufnr, { namespace = ts_js_ns })
        vim.list_extend(all_diags, ts_js_diags)
        local current_diags = vim.tbl_filter(function(d)
          return d.lnum == line
        end, all_diags)
